...
void Rond::live(double dt)
{
 ...
 if (agt==_env)
 {
  ...
  if (eq(yInter,yE-size/2)) // rayon intersecte en bas
  {
   ...
   if (agt->isA("Rond")) // Un rond a ete detecte ? Normalement pas possible
   {                     //                         car il y a un seul Rond
    Rond* rAgt=(Rond*)agt;
...
//--
void Rond::_changeColor(void)
{
 int nbColors = 12;
 setColor(_colors[randomMinMax(0,nbColors-1)]);

 vector<Agent*> v; getAllAgents("Label",v);
 if (v.size()==0) return; // Pas de label...!

 MessageColor m(getColor());
