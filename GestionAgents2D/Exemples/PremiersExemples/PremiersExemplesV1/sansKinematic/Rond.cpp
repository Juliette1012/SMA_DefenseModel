...
//--
Rond::Rond(double radius, AppliData* data) : Agent2D()
{
 newAgent();
...
{
 double side = _data->squareSize;
 bool   chgt = false;
 double x,y,theta;
 double xE,yE,tE;

 _data->square->getLocation(xE,yE,tE); // 0,0,0 ... environnement fixe
...
void Rond::live(double dt)
{
 ...
 if (agt==_data->square)
 {
  double xE,yE,tE; _data->square->getLocation(xE,yE,tE); // 0,0,0 ...
                                                         // environnement fixe
  double    size = _data->squareSize;
...
bool Rond::isEqualTo(const Rond& aR) const
{
 ...
 if (_data != aR._data)         return false;
 return true;
}
